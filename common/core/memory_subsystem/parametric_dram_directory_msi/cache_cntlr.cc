...
static UInt64 g_writestoSRAM;
static UInt64 g_writestoSTTRAM;
IntPtr eip_global;
//UInt8 l3_hit_flag;
#define WAYS_TO_SRAM    4
//UInt32 no_of_sram_ways;
   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   registerStatsMetric(name, core_id, "writestoSRAM",  &g_writestoSRAM);
   registerStatsMetric(name, core_id, "writestoSTTRAM",  &g_writestoSTTRAM);
...
LOG_ASSERT_ERROR(offset + data_length <= getCacheBlockSize(), "access until %u > %u", offset + data_length, getCacheBlockSize());
   ...
   if (!cache_hit && m_perfect)
   {
      ...
      {
         insertCacheBlock(ca_address, mem_op_type == Core::READ ? CacheState::SHARED : CacheState::MODIFIED, NULL, m_core_id, ShmemPerfModel::_USER_THREAD,100,0);
...
//l3_hit_flag=0;
      hit_where = m_next_cache_cntlr->processShmemReqFromPrevCache(this, mem_op_type, ca_address, modeled, count, Prefetch::NONE, t_start, false,eip);
...
MYLOG("copyDataFromNextLevel l%d", m_mem_component);
   ...
   else
   {
      ...
      insertCacheBlock(address, cstate, data_buf, m_core_id, ShmemPerfModel::_USER_THREAD,0,0);
...
CacheCntlr::doPrefetch(IntPtr prefetch_address, SubsecondTime t_start)
{
   ...
   HitWhere::where_t hit_where = processShmemReqFromPrevCache(this, Core::READ, prefetch_address, true, true, Prefetch::OWN, t_start, false,0);
   ...
   if (hit_where == HitWhere::MISS)
   {
      ...
      hit_where = processShmemReqFromPrevCache(this, Core::READ, prefetch_address, false, false, Prefetch::OWN, t_start, false,0);
...
HitWhere::where_t
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock, IntPtr eip)
{
   #ifdef PRIVATE_L2_OPTIMIZATION
   ...
   if (!cache_hit && m_perfect)
   {
      ...
      if (cache_block_info)
         ...
         cache_block_info = insertCacheBlock(address, mem_op_type == Core::READ ? CacheState::SHARED : CacheState::MODIFIED, NULL, m_core_id, ShmemPerfModel::_USER_THREAD,100,eip);
   }
   ...
   if (count)
   {
      ...
      /* Accounting for stores which miss in L3 */
      if ((mem_op_type == Core::WRITE)
   ...
   if (cache_hit)
   {
      ...
      if (modeled)
      {
            ...
            && (m_master->mshr[address].t_issue < t_now && m_master->mshr[address].t_complete > t_now))
         {
            ...
            //SubsecondTime latency=data_write_time;
            stats.mshr_latency += latency;
            getMemoryManager()->incrElapsedTime(latency, ShmemPerfModel::_USER_THREAD);
         }
         else
   ...
   else // !cache_hit: either data is not here, or operation on data is not permitted
   {
      ...
      if (m_next_cache_cntlr)
      {
         ...
         hit_where = m_next_cache_cntlr->processShmemReqFromPrevCache(this, mem_op_type, address, modeled, count, isPrefetch == Prefetch::NONE ? Prefetch::NONE : Prefetch::OTHER, t_issue, have_write_lock_internal,eip);
         if (hit_where != HitWhere::MISS)
         {
            cache_hit = true;
      ...
      else // last-level cache
      {
         ...
         else if (m_master->m_dram_cntlr)
         {
            ...
            else
            {
               ...
               insertCacheBlock(address, mem_op_type == Core::READ ? CacheState::SHARED : CacheState::MODIFIED, data_buf, m_core_id, ShmemPerfModel::_USER_THREAD,100,eip);
               if (isPrefetch != Prefetch::NONE)
                  getCacheBlockInfo(address)->setOption(CacheBlockInfo::PREFETCH);
 ...
 *****************************************************************************/
SharedCacheBlockInfo*
CacheCntlr::insertCacheBlock(IntPtr address, CacheState::cstate_t cstate, Byte* data_buf, core_id_t requester, ShmemPerfModel::Thread_t thread_num,UInt8 write_flag, IntPtr eip)
{
...
MYLOG("processExRepFromDramDirectory l%d", m_mem_component);
   ...
   insertCacheBlock(address, CacheState::EXCLUSIVE, data_buf, requester, ShmemPerfModel::_SIM_THREAD,3,eip_global);
   eip_global=0;
...
MYLOG("processShRepFromDramDirectory l%d", m_mem_component);
   ...
   insertCacheBlock(address, CacheState::SHARED, data_buf, requester, ShmemPerfModel::_SIM_THREAD,3,0);
}
